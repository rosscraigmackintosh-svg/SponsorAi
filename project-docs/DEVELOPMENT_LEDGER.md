...
Last updated: 2026-03-15 (Cross-page workflow pass: Board confidence + name links, Compare arrival state, Watchlist name links, property-page Board button + origin-aware back button)
...
| Sparkline charts — FanScore (small) | Active | Rendered in FanScore block on property.html via `renderSparkline()`. 120x48 SVG, async after sparks load. Also rendered at bottom of each non-suppressed Explore card (2026-03-14): `.card-spark` div in `card.js`, type-score-coloured 28px strip via `renderSpark()`. |
